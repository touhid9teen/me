...
export default function ProjectSection() {
  ...
  return (
    <div className="w-full max-w-none">
      <div
        className={`sticky top-0 z-20 -mx-6 mb-4 w-screen px-6 py-5 backdrop-blur md:-mx-12 md:px-12 lg:sr-only lg:relative lg:top-auto lg:mx-auto lg:w-full lg:px-0 lg:py-0 lg:opacity-0 bg-slate-900/75`}
      >
        <h2
      ...
      <ul className="group/list">
        {projects.map((project, index) => (
          ...
          >
            {/* Hover Background covering entire li */}
            <div
            ...
            <div className="relative z-10 grid gap-4 pb-1 sm:grid-cols-12 sm:gap-8 md:gap-4">
              {/* Project Text */}
              <div className="sm:order-2 sm:col-span-8">
                <h3>
                  <a
                    href={project.live}
                    target="_blank"
                    rel="noreferrer"
                    className={`inline-flex items-baseline font-medium leading-tight text-base transition-colors ${textColor} ${hoverText}`}
                  >
                    {project.title}
